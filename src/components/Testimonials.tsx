    ...
    <section className="py-16 md:py-20">
      <div className="container-x">
        <div className="text-center mb-14">
          <p className="section-eyebrow mb-3">{t('home.testimonials.eyebrow')}</p>
          <h2 className="section-title">{t('home.testimonials.title')}</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((r) => (
            <div key={r.id} className="card p-6">
              <div className="text-3xl text-brand-300 leading-none mb-2">&ldquo;</div>
              <p className="text-ink-700 leading-relaxed">{r.content[lang] || r.content.zh}</p>
              <div className="mt-4 pt-4 border-t border-ink-100">
                <p className="font-semibold text-ink-900">{r.author}</p>
                <p className="text-sm text-ink-500">{r.country[lang] || r.country.zh}</p>
              </div>
            </div>
          ))}
        </div>
